...
/**
 * {String} root url for Vine media
 */
Vineapple.MEDIA_URL = 'https://media.vineapp.com/';
...
Vineapple.prototype = {
  ...
  post: function (options, callback) {
    ...
    var videoUploadKey;
    var thumbnailUploadKey;

    var fOptions = {
      uri: 'https://api.vineapp.com/posts',
      headers: {
        'content-type': 'application/x-www-form-urlencoded',
        'user-agent': 'vineshell/1.0',
    ...
    var checkNext = function() {
      ...
      if (videoUploadKey && thumbnailUploadKey) {

        var body = {
          description: options.desc,
          videoUrl: videoUploadKey,
          thumbnailUrl: thumbnailUploadKey
        };

        if (options.location_id)
          body.foursquareVenueId = options.location_id;

        fOptions.form = body;

        request.post(fOptions, function(err, res, body) {
          console.log('post request got callback', body);
          if (err) console.error(err);
          else ('SUCCESSFUL POST?', res);
    ...
    var vOptions = {
      url: 'https://media.vineapp.com/upload/videos/1.3.mp4',
      headers: {
    ...
    fs.readFile(options.video, function (err, data) {
      if (err) return console.error(err);
      vOptions.body = data;
      request.put(vOptions, function (err, m, d) {
        if (err) return console.error(e);
        videoUploadKey = m.headers['x-upload-key'];
        console.log('video upload successful, upload key = ',videoUploadKey);
        checkNext();
      });
    });

    var sOptions = {
      url: 'https://media.vineapp.com/upload/thumbs/1.3.jpg',
      headers: {
    ...
    fs.readFile(options.thumbnail, function (err, data) {
      if (err) return console.error(err);
      sOptions.body = data;
      request.put(sOptions, function (err, m, d) {
        if (err) return console.error(e);
        thumbnailUploadKey = m.headers['x-upload-key'];
        console.log('thumbnail upload successful, upload key = ',thumbnailUploadKey);
        checkNext();
      });
    });
  },
  ...
  request: function (options, callback) {
    ...
    console.log("Session id = ",this.key);
